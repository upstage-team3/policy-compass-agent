...
반드시 아래 JSON 형식으로만 답하세요: {"intent": "RECOMMEND|EXPLAIN|ELIGIBILITY_CHECK|GENERAL|OUT_OF_SCOPE"}"""
...
반드시 지킬 규칙:
1. 검색 근거 없이 "반드시 신청 가능합니다", "무조건 지원됩니다" 와 같은 확정적 표현을 쓰지 마세요.
   자격 조건이 불확실하면 "신청 가능성이 높습니다", "추가 확인이 필요합니다" 와 같은 완곡한 표현을 쓰세요.
2. 최종 자격 판단이나 실제 신청 행위는 Agent가 대행하지 않으며, 공식 공고문/담당 기관을 통해
   확인하도록 안내하세요.
3. 각 추천 항목에는 사업명, 추천 이유, 지원 대상, 신청 기간, 신청 방법, 신청 전 확인 필요 조건,
   원문 링크를 포함하세요.
4. 민감 개인정보(주민등록번호, 계좌번호 등)를 요청하지 마세요."""
