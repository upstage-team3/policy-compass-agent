...
최종 갱신: 2026-07-13  
기준: LLM 중심 멀티턴 대화·Supabase 메모리·외부 API 연동 검증 반영
...
- Ruff lint/format 통과, pytest `64 passed`
...
- 청년정책은 지역·만 나이·상태·관심 지원 분야, 훈련/채용은 직무·지역,
...
현재 기준 기대 결과는 `64 passed`다. 테스트는 `tests/conftest.py`에서 외부 키를 비워 네트워크 없이 재현 가능해야 한다.
