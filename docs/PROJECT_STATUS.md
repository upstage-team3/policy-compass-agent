...
정책나침반은 사용자의 자연어 질문에서 나이, 지역, 취업/창업 상태 등을 추출하고, 기업마당 API 또는 Mock 정책 데이터를 근거로 신청 가능성이 높은 정부 지원사업 후보를 추천하는 AI Agent 서비스다.

현재 코드는 팀원이 공유한 `policy-compass-agent` 구조를 기반으로 통합했다. FastAPI 단일 서버가 API와 정적 채팅 UI를 함께 제공하며, LangGraph 기반 Agent 흐름, 세션 기반 프로필 누적, 정책 목록/검색 API, SSE 스트리밍, Mock fallback, 기업마당 API 직접 정규화 로직을 포함한다.
...
| 영역 | 상태 | 진행 내용 |
| --- | --- | --- |
| 기획 | 완료 | 프로젝트명, 문제 정의, MVP 범위, 데모 시나리오 정리 |
...
| 정책 데이터 | 1차 완료 | 팀원 스키마에 맞춘 `data/mock_policies.json` 작성 |
| 기업마당 API 연동 | 1차 완료 | `BIZINFO_API_KEY` 기반 호출 및 응답 정규화, 실패 시 Mock fallback |
| 추천 점수화 | 1차 완료 | 지역, 연령, 취업상태, 창업 여부, 신청기간 기준 점수화 |
| LLM 응답 | 진행 중 | Upstage Solar 연동 구조 있음. 정책 사실 생성 금지/근거 기반 응답 가이드라인 보강 필요 |
| 가드레일 | 1차 완료 | 확정적 표현 완화, 최종 자격 확인 안내 추가 |
| UI | 1차 완료 | FastAPI 정적 HTML 채팅 UI를 `/`에서 제공 |
| API | 1차 완료 | `/api/chat`, `/api/chat/stream`, `/api/policies`, `/api/health` 제공. 기존 `/health`, `/api/v1/chat/sync` 호환 유지 |
| 테스트 | 완료 | `23 passed`, 경고 2건은 외부 라이브러리 deprecation warning |
| Docker/GCP 배포 | 진행 전 | Dockerfile/Compose는 8000 포트 기준, VM 배포 검증 필요 |
...
6. 조건이 충분하면 Policy Search Tool이 기업마당 API 또는 Mock 데이터를 검색한다.
...
| `app/repositories/policy.py` | 기업마당 API 호출, 응답 정규화, Mock fallback |
...
| `data/mock_policies.json` | 데모용 정책 데이터 |
| `tests/` | Agent/API 테스트 |
...
- 정책 데이터: 기업마당 지원사업정보 API + Mock fallback
...
- 정책명, 신청기간, 지원내용, 신청방법, 원문 링크는 API/Mock/RAG 결과에서만 가져온다.
