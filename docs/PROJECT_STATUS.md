...
## 2026-07-10 CI/CD 및 배포 상태

GitHub Actions CI/CD는 PR merge 이후 실제 배포까지 검증 완료했다.
...
- 사용자가 PR merge
- `main` 브랜치 CI 자동 실행
- `main` CI 성공 후 CD 자동 실행
- GHCR 이미지 build/push 성공
- GCE VM SSH 배포 성공
- VM Docker Compose 기반 컨테이너 재시작 성공
- `/api/health` 기준 헬스체크 성공

운영 메모:

- CD는 노트북 Docker가 아니라 GCE VM의 Docker를 사용한다.
- VM에는 Docker와 Docker Compose plugin이 설치되어 있어야 한다.
- GitHub Secrets의 `GCE_USERNAME` 사용자가 VM에서 `docker` 명령을 실행할 수 있어야 한다.
- CD 실패 후 서버 설정만 수정한 경우 GitHub Actions에서 `Re-run failed jobs`로 재실행한다.

## 2026-07-10 개발 보완 이슈

1. 일반 대화 생성 노드에 LLM 사용 적용
   - 일반 설명/상담형 질문 전용 응답 노드를 명확히 분리한다.
   - 검색/추천이 필요한 질문과 일반 설명 질문을 분리한다.
   - LLM 사용 시에도 정책명, 금액, 날짜, 자격 조건, URL은 근거 없이 생성하지 않는다.

2. SSE 상태 문구 개선
   - 현재 모든 대화에서 거의 동일한 상태 문구가 전송된다.
   - 질문 유형별로 다른 진행 문구를 전송하도록 개선한다.
   - 훈련과정, 정책 추천, 채용 정보, 일반 설명 각각의 상태 메시지를 분리한다.

3. 라우팅 의도 분류 고도화
   - 국비지원 설명 질문과 실제 훈련과정 검색 질문을 분리한다.
   - 청년정책 추천 질문과 제도 설명 질문을 분리한다.
   - 채용공고 검색 질문과 취업 준비 일반 상담을 분리한다.
   - 조건이 부족한 검색 질문은 Tool 호출 전에 추가 질문으로 연결한다.
