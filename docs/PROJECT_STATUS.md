...
| 프로필 추출 | 2차 완료 | 최근 문맥을 참조하고 새 관심 분야는 현재 값으로 교체 |
| Tool 선택 | 2차 완료 | LLM이 선택한 데이터 Tool 하나만 호출 |
| 조건 확인 | 2차 완료 | 요청 유형별 필수 조건 질문과 원래 검색 계획 보존 |
| 응답 생성 | 3차 완료 | grounded 응답, 출처별 no-result 응답, raw payload 축소 |
...
| 테스트 | 통과 | Ruff 통과, pytest `64 passed` |
