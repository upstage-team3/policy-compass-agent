...
- `/api/chat/stream` 응답에 `status`, `token`, `done` 이벤트가 포함되는지 확인한다.
- 조건 부족 질문은 정책 검색 결과를 바로 보여주지 않고 부족 조건을 먼저 묻는다.
- 기업마당 API 필드가 비어 있을 때도 기본 안내 문구와 공식 사이트 링크로 응답이 깨지지 않는다.
- 테스트 기준은 `uv run python -m pytest` 26개 통과다.
