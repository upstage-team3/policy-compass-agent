...
- **세션 저장소**: 인메모리 `MemorySaver` 를 사용해 서버 재시작 시
  ...
  필요합니다.
- **멀티턴 프로필**: 현재 새 `interest_fields`가 기존 관심 분야와 합쳐집니다.
  분야 전환 의도를 감지해 유지·교체를 선택하는 로직이 필요합니다.
- **SSE 상태**: `status`, `token`, `done`, `error` 이벤트는 구현됐지만 질문
  유형별 상태 문구와 실제 LLM token streaming은 후속 과제입니다.
